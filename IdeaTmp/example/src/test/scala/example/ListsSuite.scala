...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers") {
    ...
    assert(sum(List(1,2,2,6,6,0,-1,-1,1,0)) === 16)
  }
  ...
  test("max of a few numbers") {
    ...
    assert(max(List(-2, -1, 0)) === 0)
    assert(max(List(-1, -2, -3)) === -1)
  }
  
  test("max of an one number") {
    assert(max(List(1)) === 1)
    assert(max(List(-1)) === -1)
    assert(max(List(0)) === 0)
  }
  
  test("max of a repeated number") {
    assert(max(List(1, 1, 4, 4, 2, 3, 2, 4, 1)) === 4)
    assert(max(List(-1, -1, 0, 4, -1, -1, 4, 6, 0, 5)) === 6)
  }
  
  test("throws exception on an empty list") {
