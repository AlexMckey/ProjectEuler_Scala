...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = asc0(this, new EmptyTrending)
  def asc0(ts: TweetSet, acc: Trending): Trending = { 
    if (ts.isEmpty) acc
    else {
      val m = ts.findMin
      asc0(ts.remove(m), acc + m)
      //asc0(ts.remove(m), new NonEmptyTrending(m, acc))
    }
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    //if (p(elem)) accu incl elem  union left.filter0(p, accu) union right.filter0(p, accu) 
    //else accu union left.filter0(p, accu) union right.filter0(p, accu)
...
object GoogleVsApple {
  ...
  val bigSet = TweetReader.allTweets
  
  val googleTweets: TweetSet = bigSet.filter(p => google.exists(p.text.contains))

  val appleTweets: TweetSet = bigSet.filter(p => apple.exists(p.text.contains))
