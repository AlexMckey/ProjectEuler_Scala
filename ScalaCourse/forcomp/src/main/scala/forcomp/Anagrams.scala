...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map({case(c,l) => (c,l.length)}).toList.sortBy({case(c,_) => c})
   ...
   */
  def occToSt(occurrences: Occurrences): String = 
    occurrences.map({case(c,i) => c.toString*i}).mkString
  
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val st = occToSt(occurrences)
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = wordOccurrences(occToSt(x) diff occToSt(y))
